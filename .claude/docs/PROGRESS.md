...
- ★ 단순화 (퍼플 외부 시각 + 사용자 호소 + 메모리 정합): `.cursor/rules/{arc,modes,file-routing}.mdc` 3개 → `archive/2026-05_simplify/cursor_rules_extra/`. handoff.md/mdc 자체도 6섹션 → 3섹션 압축. 운영 3문장 = "큰 그림은 Claude, 파일 수정은 Cursor, 막히면 다시 Claude". 활성 룰 = handoff 2개 + AGENTS.md만.
