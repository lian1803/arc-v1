...
from services.message_generator import generate_all_messages, generate_reply_message
...
class ReplyMessageRequest(BaseModel):
    stage: str    # "1차_회신" | "무응답_3일" | "무응답_7일" | "무응답_14일"
    reply_text: str = ""  # 빈 문자열 = 무응답
...
@router.post("/message/reply/{history_id}")
async def generate_reply(
    ...
    """
    대화형 다음 메시지 생성.
    stage: "1차_회신" | "무응답_3일" | "무응답_7일" | "무응답_14일"
    reply_text: 사장님이 보낸 회신 내용 (없으면 빈 문자열)
    """
    try:
        history = await db.get(DiagnosisHistory, history_id)
        if not history:
        ...
        diag_data = _build_diagnosis_dict(history)
        message = generate_reply_message(diag_data, body.stage, body.reply_text)
