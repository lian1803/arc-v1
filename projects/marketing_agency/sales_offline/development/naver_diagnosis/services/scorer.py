...
def _safe_vol(v) -> int:
    """search_volume을 안전하게 int로 변환. '< 10', None, 문자열 전부 처리."""
    if v is None:
        return 0
    try:
        return int(v)
...
class DiagnosisScorer:
    ...
    @staticmethod
    def calculate_photo_score(photo_count: int, photo_quality_score: float = 0.0) -> float:
        """
        사진 점수: 수량 40점 + 품질 60점
        photo_quality_score: 0~100 (AI 평가, 없으면 50점 기본값으로 처리)
        """
        # 수량 점수 (40점)
        if photo_count <= 0:
            quantity_score = 0.0
        ...
        # 품질 점수 (60점)
        # photo_quality_score가 0이면 AI 분석 안 된 것 → 기본 50점 (중간값)
        effective_quality = photo_quality_score if photo_quality_score > 0 else 50.0
        quality_score = (effective_quality / 100.0) * 60.0
    ...
    def calculate_review_score(
        review_count: int,
        review_sentiment_score: float = 50.0,
        owner_reply_rate: float = 0.0
    ) -> float:
        """
        리뷰 점수: 수량 40점 + 감성 35점 + 응답률 25점
        """
        # 수량 점수 (40점)
        if review_count <= 0:
            quantity_score = 0.0
        ...
        # 감성 점수 (35점)
        effective_sentiment = review_sentiment_score if review_sentiment_score > 0 else 50.0
        sentiment_score = (effective_sentiment / 100.0) * 35.0

        # 응답률 점수 (25점)
        if owner_reply_rate >= 0.9:
            reply_score = 25.0
        elif owner_reply_rate >= 0.61:
            reply_score = 20.0
        elif owner_reply_rate >= 0.31:
            reply_score = 16.0
        elif owner_reply_rate >= 0.01:
            reply_score = 8.0
        else:
            reply_score = 0.0

        return min(quantity_score + sentiment_score + reply_score, 100.0)
    ...
    def calculate_keyword_score(keywords: List) -> float:
        ...
        # 검색량 보너스 (search_volume이 str로 올 수도 있어 int 캐스팅)
        total_volume = sum(_safe_vol(kw.get("search_volume")) for kw in keywords if isinstance(kw, dict))
    ...
    def generate_improvement_points(data: Dict[str, Any]) -> List[Dict[str, str]]:
        ...
        for kw in keywords:
            if isinstance(kw, dict):
                total_volume += _safe_vol(kw.get("search_volume"))
        if total_volume == 0:
            points.append({
                "category": "keyword",
                "priority": 2,
    ...
    def calculate_estimated_lost_customers(
        ...
        # 키워드 월 검색량 합산
        total_search = sum(
            _safe_vol(kw.get("search_volume")) for kw in keywords if isinstance(kw, dict)
        )
        if not total_search:
        ...
        if competitor_avg_review > 0 and review_count < competitor_avg_review:
            ...
            # review 수가 0이면 penalty 무한대 방지용 cap (1.5)
            review_penalty = min(raw_penalty, 1.5)
        ...
        lost_value = min(max(estimated_lost, 0), 1499)

        # lost > 500 시 PDF 출력 단계에서 "최대 추정값" 라벨 표시 필요
        # (generate_html_pdf.py 에서 이 값이 max cap 도달 시 표기)
        return lost_value
    ...
    def calculate_all(cls, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # 개별 점수 계산
        photo_score = cls.calculate_photo_score(
            data.get("photo_count", 0),
            data.get("photo_quality_score", 0.0)
        )
        review_score = cls.calculate_review_score(
            data.get("review_count", 0),
            data.get("review_sentiment_score", 50.0),
            data.get("owner_reply_rate", 0.0)
        )
