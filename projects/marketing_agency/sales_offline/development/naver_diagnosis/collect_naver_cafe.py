"""네이버 카페 검색 → 010 추출 (블로그와 다른 커뮤니티 소스). 6r × 11kw."""
...
async def search_cafe(ctx, region: str, kw: str) -> list[dict]:
    results, seen_local = [], set()
    pg = await ctx.new_page()
    try:
        url = (f"https://search.naver.com/search.naver"
               f"?where=cafe&query={region}+{kw}+전화&display=10")
        ...
        for m in PHONE_RE.finditer(html):
            ...
            if len(d) != 11 or d in seen_local:
                continue
            seen_local.add(d)
            ctx_str = html[max(0, m.start() - 200):m.start()]
            nm = re.search(r'"([가-힣a-zA-Z0-9 ]{2,20})"', ctx_str)
            results.append({
...
async def main():
    ...
    async with async_playwright() as pw:
        ...
        for i, region in enumerate(REGIONS):
            for j, kw in enumerate(KEYWORDS):
                ...
                recs = await search_cafe(ctx, region, kw)
    ...
    for out in [LEAD_DB / f"북부수도권_네이버카페_{TS}.xlsx",
                DESKTOP / f"북부수도권_네이버카페_{TS}.xlsx"]:
        wb = openpyxl.Workbook(); ws = wb.active; ws.append(hdrs)
        for r in all_r:
            ws.append([r["region"], r["keyword"], r["name"], r["phone"]])
        wb.save(str(out))

    bk = LEAD_DB / f"북부수도권_네이버카페_{TS}.json"
